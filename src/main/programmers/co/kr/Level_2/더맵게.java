...
public class 더맵게 {
    ...
    public int solution(int[] scoville, int K) {
        int answer = 0;
        List<Integer> temp = Arrays.stream(scoville).boxed().sorted().collect(Collectors.toList());

        while(temp.get(0) < K) {
            temp.add(mix(temp.get(0), temp.get(1)));
            temp.remove(0);
            temp.remove(0);
    ...
    public int 참고할만한_풀이(int[] scoville, int K) {
        int answer = 0;
