...
public class 오픈채팅방_KAKAO2018 {
    private static final String JOIN_MESSAGE = "님이 들어왔습니다.";
    private static final String LEAVE_MESSAGE = "님이 나갔습니다.";

    //TODO: 성능이 괜찮은건지? 더 효율적인 코드는 없는지?
    public String[] solution(String[] record) {
        ...
        for (String input : record) {
            String[] parsed = input.split(" ");
            if(input.contains("Enter")) {
                result.add(idSet.get(parsed[1]) + JOIN_MESSAGE);
            }

            if(input.contains("Leave")) {
                result.add(idSet.get(parsed[1]) + LEAVE_MESSAGE);
            }
        }
